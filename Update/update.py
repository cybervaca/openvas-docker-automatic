...
with open("/opt/gvm/Update/version.txt", "r") as f:
    versionanterior = f.read()
if (control==1):
    with open("/opt/gvm/Update/version.txt","w") as f:
        if(versionanterior != soloversion):
            print(versionanterior)
            f.write(soloversion)
            salida = os.system("bash /opt/gvm/Update/update-scanner.sh " + soloversion)
            print(salida)
